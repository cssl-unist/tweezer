...
#else
		//Temporary disable dynamic allocation. Using stack now
		//unsigned char *temp_contents = new unsigned char[block_contents.size() + kBlockTrailerSize];
		if(block_contents.size() + kBlockTrailerSize > Encryption_buffer.size())
			Encryption_buffer.reserve(block_contents.size() + kBlockTrailerSize);
    memcpy((char*)Encryption_buffer.data(),block_contents.data(),block_contents.size());
    memcpy((char*)Encryption_buffer.data() + block_contents.size(),trailer,kBlockTrailerSize);
    Slice enc_contents((const char *)Encryption_buffer.data(),block_contents.size() + kBlockTrailerSize);
		Encryption(enc_contents,memtable_key,gcm_iv,gcm_aad);
		unsigned char hmac[48];
		digest(hmac,enc_contents,memtable_key);
    r->hmacs.push_back(std::string((char*)hmac,48));
    io_s = r->file->Append(enc_contents);
    handle->set_hmac((uint64_t)r->hmacs.size()-1);
#endif
    if (io_s.ok()) {
      assert(s.ok());
...
#ifdef BLOCK_ENC
	footer.set_hmac(r->hmacs);
	footer.set_hmac_offset(r->offset);
#endif
