...
#endif
#ifdef BLOCK_ENC
	if (GetVarint64(input, &offset_) && GetVarint64(input, &size_) && GetFixed64(input,&hmac_offset_)) {
#endif
    return Status::OK();
  } else {
...
#ifdef BLOCK_ENC
    if(hmacs.size() > 0){
      for(int i = 0;i <(int)hmacs.size() ; i++){
        //Later have to consider littleendian
        dst->append(hmacs[i]);
      }
    }
    PutFixed64(dst,hmac_offset_);
...
#ifdef BLOCK_ENC
	hmac_offset_ = DecodeFixed64(input->data());
	input->remove_prefix(8);
#endif

#ifdef SEPERATE_KEY
  int id_ = DecodeFixed32(input->data() + input->size() - 16);
  set_key(id_);
#endif
...
Status ReadFooterFromFile(const IOOptions& opts, RandomAccessFileReader* file,
  ...
  Slice footer_input;
#if defined(BLOCK_ENC)
  size_t read_offset =
      (file_size > Footer::kMaxEncodedLength)
          ? static_cast<size_t>(file_size - Footer::kMaxEncodedLength) - 8
          : 0;
#else
	size_t read_offset =
      (file_size > Footer::kMaxEncodedLength)
...
#else
			!prefetch_buffer->TryReadFromCache(
          IOOptions(), read_offset, Footer::kMaxEncodedLength + 8, &footer_input)) {
#endif
...
#else
			s = file->Read(opts, read_offset, Footer::kMaxEncodedLength + 8,
                     &footer_input, nullptr, &internal_buf);
#endif
    } else {
#if defined(BLOCK_ENC)
			footer_buf.reserve(Footer::kMaxEncodedLength + 8);
      s = file->Read(opts, read_offset, Footer::kMaxEncodedLength + 8,
...
#ifdef BLOCK_ENC
	int hmac_length = read_offset - footer->hmac_offset();
  char *hmac_ = new char[hmac_length];
  Slice Footer_hmac;
  s = file->Read(opts,footer->hmac_offset(),hmac_length,&Footer_hmac,hmac_,nullptr);
  std::vector<std::string> hmacs;
  for(int i = 0; i < (int)(hmac_length/48);i++){
		char hmac[48];
		memcpy(hmac,hmac_ + (i * 48),48);
    hmacs.push_back(std::string(hmac,48));
	}
	char hmac_test[48];
	digest((unsigned char*)hmac_test,Slice(hmac_,hmac_length),memtable_key);
	delete [] hmac_;
  footer->set_hmac(hmacs);
#endif
